...
const getLandInfo = async (landNumber) => {
  ...
  const response = await fetch(
    `https://pixels-server.pixels.xyz/game/findroom/pixelsNFTFarm-${landNumber}/99?v=${timestamp}`
  );
...
client.on("messageCreate", async (message) => {
  ...
  if (isNaN(landNumber)) {
    return message.reply(
      "Por favor, forneça um número de land válido como argumento."
    );
  }
  ...
  while (true) {
    ...
    if (messagesList.length > 0) {
      ...
      const channel = client.channels.cache.get("1223740297658499073");
      if (channel) {
